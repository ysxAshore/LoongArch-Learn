...
module id_stage (
  ...
  assign br_cancel = br_taken & id_ready_go; //需要结合id_ready_go 因为判断了exe、wb、mem的
  ...
  assign id_regW = ~inst_st_w & ~inst_st_b & ~inst_st_h & ~inst_beq & ~inst_bne & ~inst_blt & ~inst_bge & ~inst_bltu & ~inst_bgeu & ~inst_b & ~inst_syscall & ~inst_break & ~inst_ertn & ~inst_ine & ~id_ADEF_EXCP & id_valid;
  ...
  wire mem_data_ok_stall = ~mem_ready_go & id_valid & ((need_rj & regAddrA != 5'b0 & (exe_regWAddr == regAddrA | mem_regWAddr == regAddrA | wb_regWAddr == regAddrA))|
                                  (need_rkd & regAddrB != 5'b0 & (exe_regWAddr == regAddrA | mem_regWAddr == regAddrB | wb_regWAddr == regAddrB)));
