...
-   [1 验证5条指令的单周期CPU](#1-验证5条指令的单周期CPU)
    -   [1.1 快速上手](#11-快速上手)
        ...
        -   [1.1.2 填空minicpu\_env/miniCPU/minicpu\_top.v文件](#112-填空minicpu_envminiCPUminicpu_topv文件)
...
### 1.1.2 填空minicpu\_env/miniCPU/minicpu\_top.v文件
...
module minicpu_top(
    ...
    output wire [31:0] inst_sram_wdata,//写数据可以是任意值，这里设置为了0
...
//ADDW 31-26值为0,25-22值为0,21-20是1,19-15是0
//ADDIW 31-26值为0,25-22值是10即十六进制a
//LDW 31-26值为10即a,25-22值为2
//STW 31-26值为10即a,25-22值为6
//BNE 31-26值为6'h17
...
assign src2_is_imm   = inst_addi_w | inst_ld_w | inst_st_w;//在这里实现立即数选择信号 ADDIW、LDW、STW的第二个操作数需要是立即数
...
assign br_offs   = {{14{i16[15]}},i16,2'b0};//在这里完成br_offs信号的生成 低位补2位0再符号扩展
...
1.  clk\_pll IP 被上锁导致`Run Sumlation`出现Module \<clk\_pll> not found while processing module instance \<pll.clk\_pll>
    ...
    1.  点击Report–>Report IP Status
        ...
        ![](image/image_OU_rPuFSCK.png)
    2.  勾选被锁的IP核，点击Upgrade Selected

        ![](image/image_L2BPXtox8N.png)
