...
reg         request_buffer_icacop_en  ;
reg [ 1:0]  request_buffer_icacop_mode;
reg [19:0]  request_buffer_icacop_tag ;
reg [ 7:0]  request_buffer_icacop_index;
reg [ 3:0]  request_buffer_icacop_offset;
...
assign mode0=icacop_mode==2'b0;
assign mode1=icacop_mode==2'b1;
assign mode2=icacop_mode==2'b10;
assign real_tag   =icacop_en? icacop_tag   :tag;
assign real_index =icacop_en? icacop_index :index;
assign real_offset=icacop_en? icacop_offset:offset;
...
always @(posedge clk) begin
    if (reset) begin
		...
		request_buffer_icacop_en  <=  1'b0;
		request_buffer_icacop_mode<=  2'b0;
		request_buffer_icacop_tag <= 20'b0;
		request_buffer_icacop_index<= 8'b0;
		request_buffer_icacop_offset<=4'b0;
        ...
        main_idle: begin
            if ((req_or_inst_valid || icacop_en) && main_idle2lookup) begin
                main_state <= main_lookup;

                request_buffer_op         <= op        ;
				request_buffer_uncache    <= uncache && !icacop_en;
                ...
                request_buffer_icacop_en  <= icacop_en;
		        request_buffer_icacop_mode<= icacop_mode;
		        request_buffer_icacop_tag <= icacop_tag;
		        request_buffer_icacop_index<=icacop_index;
		        request_buffer_icacop_offset<=icacop_offset;
...
assign icacop_ok= (main_state==main_idle) && icacop_en;
...
assign addr_ok = (main_state_is_idle && main_idle2lookup) || (main_state_is_lookup && main_lookup2lookup) && !request_buffer_icacop_en; //request can be get
...
assign rd_type = request_buffer_uncache? 3'b010:3'b100;
...
assign data_ok = ((main_state_is_lookup && (cache_hit)) || 
                  (main_state_is_refill && (ret_valid && ((miss_buffer_ret_num == request_buffer_offset[3:2]) || request_buffer_uncache  )))) ; //when rd_req is not set, set data_ok directly.
...
assign way0_cacop_hit  = way0_tagv_douta[0] && (request_buffer_icacop_tag==way0_tagv_douta[20:1] && request_buffer_icacop_mode==2'b10) && request_buffer_icacop_en; //this signal will not maintain
assign way1_cacop_hit  = way1_tagv_douta[0] && (request_buffer_icacop_tag==way1_tagv_douta[20:1] && request_buffer_icacop_mode==2'b10) && request_buffer_icacop_en;                                         
...
assign tagv_addra = (addr_ok || icacop_en && main_state_is_idle)?                    real_index         :          
                    (request_buffer_icacop_mode==2'b10 && cache_cacop_hit)? request_buffer_icacop_index :
...
/*assign tagv_wea_en = (main_state_is_refill && (ret_valid && ret_last)) || ((mode0 || mode1) && main_idle && icacop_en) || (request_buffer_icacop_mode==2'b10 && cache_cacop_hit);

assign way0_tagv_wea = (!miss_buffer_replace_way || !icacop_offset[0] && (mode0 || mode1) && icacop_en || way0_cacop_hit) && tagv_wea_en; //wirte at last 4B
assign way1_tagv_wea = ( miss_buffer_replace_way ||  icacop_offset[0] && (mode0 || mode1) && icacop_en || way1_cacop_hit) && tagv_wea_en;*/

assign way0_tagv_wea = !miss_buffer_replace_way && (main_state_is_refill && (ret_valid && ret_last)) || 
                       !icacop_offset[0] && (mode0 || mode1) && icacop_en && main_state_is_idle || way0_cacop_hit && (request_buffer_icacop_mode==2'b10);
assign way1_tagv_wea =  miss_buffer_replace_way && (main_state_is_refill && (ret_valid && ret_last)) || 
                        icacop_offset[0] && (mode0 || mode1) && icacop_en && main_state_is_idle || way1_cacop_hit && (request_buffer_icacop_mode==2'b10); 

/*===============================tagv dina logic=================================*/

assign tagv_dina = ((mode0 || mode1) && main_state_is_idle && icacop_en) || (request_buffer_icacop_mode==2'b10 && cache_cacop_hit)? 21'b0:{request_buffer_tag, 1'b1};
