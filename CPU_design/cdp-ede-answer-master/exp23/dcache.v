...
reg         request_buffer_dcacop_en  ;
reg [ 1:0]  request_buffer_dcacop_mode;
...
wire      mode1;
wire      mode2;
assign mode0=dcacop_mode==2'b0;
assign mode1=dcacop_mode==2'b1;
assign mode2=dcacop_mode==2'b10;
...
always @(posedge clk) begin
    if (reset) begin
		...
		request_buffer_dcacop_en  <=  1'b0;
	    request_buffer_dcacop_mode<=  2'b0;
        ...
        main_idle: begin
            if ((req_or_inst_valid || dcacop_en) && main_idle2lookup) begin
                main_state <= main_lookup;

                request_buffer_op         <= op        ;
				request_buffer_uncache    <= uncache && !dcacop_en;
				...
				request_buffer_dcacop_en  <= dcacop_en;
		        request_buffer_dcacop_mode<= dcacop_mode;
        ...
        main_lookup: begin
            ...
            else if (!cache_hit || request_buffer_dcacop_en) begin
        ...
        main_miss: begin
            if (wr_rdy) begin
                ...
                wr_req <= (replace_d && replace_v && !request_buffer_uncache) || uncache_wr || (request_buffer_dcacop_mode==2'b10 && cache_hit_r && request_buffer_dcacop_en) || 
				          (request_buffer_dcacop_mode==2'b1 && request_buffer_dcacop_en);
                          
            end
        end
        main_replace: begin
            if (rd_rdy || uncache_wr || request_buffer_dcacop_en) begin
        ...
        main_refill: begin
            if ((ret_valid && ret_last) || !rd_req_buffer || uncache_wr || request_buffer_dcacop_en) begin   //when rd_req is not set, go to next state directly
                main_state <= main_idle;
            end
            else begin
                if (ret_valid) begin
...
assign dcacop_ok= main_state_is_replace && request_buffer_dcacop_en;
...
assign cache_hit = (way0_hit || way1_hit) && (!request_buffer_uncache || request_buffer_dcacop_en);
...
assign replace_way = (request_buffer_dcacop_mode==2'b1 && request_buffer_dcacop_en)                ? request_buffer_offset[0]:
                     (request_buffer_dcacop_mode==2'b10 && request_buffer_dcacop_en && cache_hit_r)? way1_hit                :
                                                                                                     invalid_way   ;
...
assign uncache_wr=request_buffer_uncache && request_buffer_op && !request_buffer_dcacop_en;
...
assign rd_req  = main_state_is_replace && !uncache_wr && !request_buffer_dcacop_en;
...
/*assign tagv_wea_en = main_state_is_refill && (ret_valid && ret_last) || ((mode0 || mode1) && main_state_is_idle && dcacop_en) || (request_buffer_dcacop_mode==2'b10 && cache_hit && request_buffer_dcacop_en);

assign way0_tagv_wea = (!miss_buffer_replace_way || !offset[0] && (mode0 || mode1) && dcacop_en || way0_hit) && tagv_wea_en; //wirte at last 4B
assign way1_tagv_wea = ( miss_buffer_replace_way ||  offset[0] && (mode0 || mode1) && dcacop_en || way1_hit) && tagv_wea_en;*/

assign way0_tagv_wea = !miss_buffer_replace_way && (main_state_is_refill && (ret_valid && ret_last)) || 
                       !request_buffer_offset[0] && (request_buffer_dcacop_mode==2'b0 || request_buffer_dcacop_mode==2'b1) && request_buffer_dcacop_en && main_state_is_replace || 
					    way0_hit_r && (request_buffer_dcacop_mode==2'b10) && request_buffer_dcacop_en && main_state_is_replace;
assign way1_tagv_wea =  miss_buffer_replace_way && (main_state_is_refill && (ret_valid && ret_last)) ||  
                        request_buffer_offset[0] && (request_buffer_dcacop_mode==2'b0 || request_buffer_dcacop_mode==2'b1) && request_buffer_dcacop_en && main_state_is_replace ||
						way1_hit_r && (request_buffer_dcacop_mode==2'b10) && request_buffer_dcacop_en && main_state_is_replace;

/*===============================tagv dina logic=================================*/

assign tagv_dina = ((request_buffer_dcacop_mode==2'b0 || request_buffer_dcacop_mode==2'b1) && main_state_is_replace && request_buffer_dcacop_en) ||
                    (request_buffer_dcacop_mode==2'b10 && cache_hit_r && request_buffer_dcacop_en && main_state_is_replace)? 21'b0:{request_buffer_tag, 1'b1};
