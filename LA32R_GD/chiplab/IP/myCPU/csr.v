...
module csr (
  ...
  always @(posedge clk) begin
    if (~resetn) begin
      ...
      if (ticlr_wen & csrWData[`CLR]) begin
        csr_estat[11] <= 1'b0;
      end else if(tcfg_wen) begin
        timer_en <= csrWData[`EN];
      end else if(timer_en & csr_tval == 32'h0) begin
        csr_estat[11] <= 1'b1;
  ...
  always @(posedge clk) begin
    if (~resetn) begin
      ...
      if (csrWData[`WCLLB] == 1'b1) begin //不写csrWData[`WCLLB]
        llbit <= 1'b0;
      end
  ...
  //tlbidx
  parameter NUM = 24-$clog2(`TLB_NUM);
  parameter ZERO = 0;
  always @(posedge clk) begin
    if (~resetn) begin
      csr_tlbidx[23:$clog2(`TLB_NUM)] <= ZERO[NUM-1:0];
      csr_tlbidx[30] <= 1'b0;
  ...
  always @(posedge clk) begin
    if (tcfg_wen) begin //只要写tcfg时就设置csr_tval
